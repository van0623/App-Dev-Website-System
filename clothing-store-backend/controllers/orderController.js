...
// Create a new order
const createOrder = (req, res) => {
  const { userId, cartItems, totalAmount, shippingInfo } = req.body;

  // Build shippingAddress string from shippingInfo object
  const shippingAddress = `${shippingInfo.address}, ${shippingInfo.city}, ${shippingInfo.zipCode}`;

  // Insert order with products as JSON string, and default status 'Pending'
  const query = 'INSERT INTO orders (user_id, products, total_amount, shipping_address, status) VALUES (?, ?, ?, ?, ?)';

  db.query(query, [userId, JSON.stringify(cartItems), totalAmount, shippingAddress, 'Pending'], (err, result) => {
    if (err) {
      console.error('Error inserting order:', err);
      return res.status(500).json({ message: 'Failed to create order' });
    }
